...
rule epibac_summary:
    input:
        counts=[f"{OUTDIR}/qc/count_reads/{sample}_counts.txt" for sample in samples_df.index],
        amrfinder=[f"{OUTDIR}/amr_mlst/{sample}_amrfinder.tsv" for sample in samples_df[samples_df["illumina_r2"].notnull()].index],
        mlst=[f"{OUTDIR}/amr_mlst/{sample}_mlst.tsv" for sample in samples_df[samples_df["illumina_r2"].notnull()].index],
        resfinder=[f"{OUTDIR}/amr_mlst/resfinder/{sample}/ResFinder_results.txt" for sample in samples_df[samples_df["illumina_r2"].notnull()].index]
    output:
...
# Esta regla solo se ejecuta si el modo es "gva"
if config.get("mode") == "gva":
    rule epibac_summary_gestlab:
        ...
        run:
            ...
            for col in ["SCOPE_core", "GENE_resfinder"]:
