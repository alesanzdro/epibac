...
rule epibac_summary_gestlab:
    ...
    run:
        ...
        def build_path(row):
            ...
            storage_cabinet = config.get("mode_config", {}).get("gva", {}).get("storage_cabinet", "\\\\NLSAR\\deposito")
        ...
        df_paths['dest_path_illumina'] = df_merged.apply(
            lambda row: build_path(row) if row['OBS_MET_WGS'] in ["ILLUMINA", "HYBRID"] else pd.NA, 
            axis=1
        )
        df_paths['dest_path_nanopore'] = df_merged.apply(
            lambda row: f"{config.get('mode_config', {}).get('gva', {}).get('storage_cabinet', '\\\\NLSAR\\deposito')}\\CVA_{row['CARRERA'].split('_')[1][:4]}\\nanopore\\{row['CARRERA']}" 
            if row['OBS_MET_WGS'] in ["NANOPORE", "HYBRID"] else pd.NA, 
            axis=1
        )
        
        # Guardar información de rutas para posible uso en un script de copia
        paths_file = os.path.join(os.path.dirname(output.gestlab_report), f"{DATE}_paths_for_copy.csv")
