...
def rsync_copy(source, destination):
    ...
    try:
        cmd = ["rsync", "-avh", "--progress", "--partial", "--checksum", f"{source}/", f"{destination}/"]
...
def find_fastq_files(sample_id, outdir, fastq_info):
    ...
    if isinstance(fastq_info, dict):
                ...
                for directory in search_dirs:
                    ...
                    if matches:
                        ...
                        if key == "ILLUMINA_R1":
                            ...
                            fastq_files["ONT"].extend(matches)
    else:
        ...
        for directory in search_dirs:
            ...
            all_files = glob.glob(f"{directory}/{sample_id}*.fastq.gz") + \
                        glob.glob(f"{directory}/{sample_id}*.fq.gz")
...
def determine_seq_method(row):
    ...
    has_illumina = (('ILLUMINA_R1' in row and pd.notna(row['ILLUMINA_R1']) and row['ILLUMINA_R1'] != "") or 
                   ('ILLUMINA_R2' in row and pd.notna(row['ILLUMINA_R2']) and row['ILLUMINA_R2'] != ""))
    has_nanopore = ('ONT' in row and pd.notna(row['ONT']) and row['ONT'] != "")
...
def main():
    ...
    if len(sys.argv) != 7:
        print(f"Uso: {sys.argv[0]} gestlab_report report_tsv report_xlsx output_log outdir tag_run", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        if mode == "gva":
            ...
            if storage_cabinet and not os.path.exists(storage_cabinet):
                ...
                print(f"Los archivos se dejarán en la carpeta temporal: {os.path.join(outdir, tag_run)}", file=sys.stderr)
                storage_cabinet = None
    except Exception as e:
    ...
    id_col = None
    if mode == "gva":
        id_col = config.get("params", {}).get("mode_config", {}).get("gva", {}).get("primary_id_column", "id2")
    else:
        id_col = config.get("params", {}).get("mode_config", {}).get("normal", {}).get("primary_id_column", "id")
    
    # Verificar que la columna ID exista en el informe
    if id_col not in df_gestlab.columns:
        print(f"Error: Columna {id_col} no encontrada en el informe GESTLAB", file=sys.stderr)
        # Buscar alternativas
        alt_cols = [col for col in df_gestlab.columns if col.lower() in ['id', 'id2', 'codigo_muestra', 'codigo_muestra_origen']]
        if alt_cols:
            id_col = alt_cols[0]
    ...
    try:
        shutil.copy2(gestlab_report, os.path.join(carrera_dir, f"{tag_run}_EPIBAC_GESTLAB.csv"))
    ...
    if mode == "gva" and storage_cabinet and hosp:
        ...
        if has_illumina:
            ...
            if not create_directory(dest_path):
                log_entries.append(f"Error: No se pudo crear directorio destino Illumina {dest_path}")
            else:
                # Ejecutar rsync para copiar al destino final
                success, message = rsync_copy(carrera_dir, dest_path)
                if success:
                    log_entries.append(f"Éxito: Archivos copiados correctamente a {dest_path}")
                else:
        ...
        if has_nanopore:
            ...
            if not create_directory(dest_path):
                log_entries.append(f"Error: No se pudo crear directorio destino Nanopore {dest_path}")
            else:
                # Ejecutar rsync para copiar al destino final
                success, message = rsync_copy(carrera_dir, dest_path)
                if success:
