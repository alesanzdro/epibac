...
def setup_perl_environment():
    ...
    if os.path.exists(perl_lib):
        ...
        os.environ["PATH"] = (
            f"{os.path.join(base_dir, 'bin')}:{os.environ.get('PATH', '')}"
        )
...
try:
    ...
    if all_files_exist:
        logger.info(
            "Todos los archivos necesarios ya existen. Omitiendo descarga y configuración."
        )
    ...
    if not hmm_index.exists():
        ...
        download_file(
            "https://ftp.ncbi.nlm.nih.gov/hmm/current/hmm_PGAP.HMM.tgz", tgz_path
        )
        ...
        if not hmm_pgap_dir.exists() or not any(hmm_pgap_dir.iterdir()):
            logger.error(
                f"El directorio {hmm_pgap_dir} no existe o está vacío después de la extracción"
            )
            raise FileNotFoundError(
                f"Extracción fallida: no se encontraron archivos HMM"
            )
        ...
        try:
            ...
            logger.error(
                f"Error ejecutando hmmpress: {e.stderr.decode() if hasattr(e, 'stderr') else str(e)}"
            )
            ...
            if hmm_file.exists():
                ...
                try:
                    ...
                    logger.warning(
                        "No se pudieron leer las primeras líneas del archivo"
                    )
    ...
    for cm_file in snakemake.params.cm_files:
        ...
        if not cm_path.exists():
            ...
            if not cm_path.exists() or cm_path.stat().st_size == 0:
                logger.warning(
                    f"Archivo CM {cm_file} no se descargó correctamente, intentando URL alternativa"
                )
                download_file(
    ...
    try:
        ...
        which_result = subprocess.run(
            ["which", "prokka"], capture_output=True, text=True
        )
        ...
        logger.error(
            f"Error configurando Prokka: {e.stderr if hasattr(e, 'stderr') else str(e)}"
        )
        logger.error(f"Comando: prokka --cpus 1 --dbdir {db_dir} --setupdb")
        logger.error(
            f"Código de salida: {e.returncode if hasattr(e, 'returncode') else 'desconocido'}"
        )
    ...
    if not (db_dir / "PGAP.hmm").exists():
        logger.error(
            "Archivo crítico PGAP.hmm no encontrado, la instalación puede estar incompleta"
        )
    else:
        logger.info(
            f"Archivo PGAP.hmm encontrado, tamaño: {(db_dir / 'PGAP.hmm').stat().st_size} bytes"
        )
    ...
    for ext in [".h3f", ".h3i", ".h3m", ".h3p"]:
        if not (db_dir / f"PGAP.hmm{ext}").exists():
            logger.warning(
                f"Índice {ext} no encontrado, el rendimiento puede verse afectado"
            )
