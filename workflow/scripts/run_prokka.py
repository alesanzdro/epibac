    ...
    if snakemake.params.skip:
        ...
        if os.path.exists("/opt/conda"):
            ...
            env["PERL5LIB"] = os.path.join(
                os.environ["CONDA_PREFIX"], "lib/perl5/site_perl"
            )
            env["PATH"] = (
        ...
        if (
            not os.path.exists(snakemake.input.fasta)
            or os.path.getsize(snakemake.input.fasta) == 0
        ):
            ...
            log(
                "Intentando ejecutar Prokka con opción --compliant para evitar parallel"
            )
            cmd = [
                "prokka",
                "--cpus",
                ...
                snakemake.params.prefix,
                "--hmms",
                os.path.join(snakemake.params.db_dir, "PGAP.hmm"),
                "--outdir",
            ...
            try:
                result = subprocess.run(
                    cmd, stdout=log_file, stderr=subprocess.STDOUT, env=env, check=False
                )
                ...
                if result.returncode == 0:
                    ...
                    log(
                        f"Error al ejecutar Prokka con --compliant. Código de salida: {result.returncode}"
                    )
                    ...
                    if result.returncode == 0:
                        ...
                        log(
                            f"Error al ejecutar Prokka. Código de salida: {result.returncode}"
                        )
                        log(
                            "No se pudo ejecutar Prokka. Continuando el pipeline sin anotación."
                        )
            ...
            faa_src = os.path.join(
                snakemake.output.dir, f"{snakemake.params.prefix}.faa"
            )
            gff_src = os.path.join(
                snakemake.output.dir, f"{snakemake.params.prefix}.gff"
            )
